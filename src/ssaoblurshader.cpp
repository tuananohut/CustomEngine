#include "headers/ssaoblurshader.h"
...
SSAOBlurShader::SSAOBlurShader(const SSAOBlurShader& other) {}

SSAOBlurShader::~SSAOBlurShader() {}

bool SSAOBlurShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	wchar_t vsFilename[128], psFilename[128];
	int error;
	bool result;

	error = wcscpy_s(vsFilename, 128, L"src/shaders/ssaoblur.vs");
	if (error != 0)
	{
		return false;
	}

	error = wcscpy_s(psFilename, 128, L"src/shaders/ssaoblur.ps");
	if (error != 0)
	{
		return false;
	}
...
void SSAOBlurShader::Shutdown()
{
	ShutdownShader();
}

bool SSAOBlurShader::Render(ID3D11DeviceContext* deviceContext,
							int indexCount,
							XMMATRIX worldMatrix,
							XMMATRIX viewMatrix,
							XMMATRIX projectionMatrix,
							ID3D11ShaderResourceView* ssaoTexture,
							ID3D11ShaderResourceView* normalDepthTexture, 
							int screenWidth,
							int screenHeight,
							int blurType)
{
...
bool SSAOBlurShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
...
void SSAOBlurShader::ShutdownShader()
{
	if (m_screenBuffer)
	{
...
void SSAOBlurShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool SSAOBlurShader::SetShaderParameters(ID3D11DeviceContext* deviceContext,
										 XMMATRIX worldMatrix,
										 XMMATRIX viewMatrix,
										 XMMATRIX projectionMatrix,
										 ID3D11ShaderResourceView* ssaoTexture,
										 ...
										 int blurType) 
{
	...
	if (blurType == 0)
	{
		dataPtr2->blurType = 0.f;
	}

	else
	{
		dataPtr2->blurType = 1.f;
	}
...
void SSAOBlurShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
