...
bool SpecMapShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"src/shaders/specmap.vs");
	if (error != 0)
	{
		return false;
	}

	error = wcscpy_s(psFilename, 128, L"src/shaders/specmap.ps");
	if (error)
	{
		return false;
	}
...
bool SpecMapShader::Render(ID3D11DeviceContext* deviceContext, 
						   int indexCount,
						   ...
						   float specularPower)
{
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture1, texture2, texture3, lightDirection, diffuseColor, cameraPosition, specularColor, specularPower);
	if (!result)
	{
		return false;
	}
...
bool SpecMapShader::InitializeShader(ID3D11Device* device, 
	                                 HWND hwnd,
									 WCHAR* vsFilename,
									 WCHAR* psFilename)
{
...
bool SpecMapShader::SetShaderParameters(ID3D11DeviceContext* deviceContext,
										XMMATRIX worldMatrix,
										XMMATRIX viewMatrix,
										XMMATRIX projectionMatrix,
										ID3D11ShaderResourceView* texture1,
