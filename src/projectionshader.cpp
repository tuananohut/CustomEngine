#include "headers/projectionshader.h"
...
bool ProjectionShader::Initialize(ID3D11Device* device, HWND hwnd)
{
...
void ProjectionShader::Shutdown()
{
	ShutdownShader();
}
...
bool ProjectionShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	result = D3DCompileFromFile(psFilename, NULL, NULL, "ProjectionPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
		if (errorMessage)
		{
...
void ProjectionShader::ShutdownShader()
{
	if (m_sampleState)
	{
...
void ProjectionShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
void ProjectionShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
