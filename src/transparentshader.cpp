#include "headers/transparentshader.h"
...
bool TransparentShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"../CustomEngine/src/shaders/transparent.vs");
	if (error != 0)
	{
		return false;
	}

	error = wcscpy_s(psFilename, 128, L"../CustomEngine/src/shaders/transparent.ps");
	if (error != 0)
	{
		return false;
	}
...
bool TransparentShader::Render(ID3D11DeviceContext* deviceContext,
							   int indexCount, 
...
bool TransparentShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
...
bool TransparentShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, 
											XMMATRIX worldMatrix, 
											XMMATRIX viewMatrix,
											XMMATRIX projectionMatrix, 
											ID3D11ShaderResourceView* texture, 
