...
bool GlassShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"../CustomEngine/src/shaders/glass.vs");
	if (error != 0)
	{
		return false;
	}

	error = wcscpy_s(psFilename, 128, L"../CustomEngine/src/shaders/glass.ps");
	if (error != 0)
	{
		return false;
	}
...
bool GlassShader::Render(ID3D11DeviceContext* deviceContext,
						 int indexCount, 
...
bool GlassShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
...
bool GlassShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, 
									  XMMATRIX worldMatrix, 
									  XMMATRIX viewMatrix, 
									  XMMATRIX projectionMatrix, 
									  ID3D11ShaderResourceView* colorTexture, 
