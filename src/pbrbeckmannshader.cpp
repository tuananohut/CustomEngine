...
PBRBeckmannShader::PBRBeckmannShader()
{
...
PBRBeckmannShader::PBRBeckmannShader(const PBRBeckmannShader& other) {}

PBRBeckmannShader::~PBRBeckmannShader() {}

bool PBRBeckmannShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(psFilename, 128, L"src/shaders/pbr_beckmann.ps");
	if (error != 0)
	{
		return false;
	}
...
void PBRBeckmannShader::Shutdown()
{
	ShutdownShader();
}

bool PBRBeckmannShader::Render(ID3D11DeviceContext* deviceContext,
	int indexCount,
...
bool PBRBeckmannShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
...
void PBRBeckmannShader::ShutdownShader()
{
	if (m_lightBuffer)
	{
...
void PBRBeckmannShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool PBRBeckmannShader::SetShaderParameters(ID3D11DeviceContext* deviceContext,
	XMMATRIX worldMatrix,
	XMMATRIX viewMatrix,
	XMMATRIX projectionMatrix,
	ID3D11ShaderResourceView* diffuseTexture,
...
void PBRBeckmannShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
